...
configOanda = open( os.path.join(configDir,'Oanda.config'), 'r')
GSheet, Token, AccountID, *rest = configOanda.read().splitlines()
configOanda.close()
configGS = os.path.join(configDir,'GS.json')
UpdateSeconds = 1
...
gs = pygsheets.authorize(service_file=configGS)
