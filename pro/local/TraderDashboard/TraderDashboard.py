...
Token = input("Enter Oanda's API Token: ")
AccountID = input("Enter Oanda's Account ID: ")
GSheet = input("Enter GoogleSheet's name: ")
UpdateSeconds = int(input("How often do you want to update the dashboard? (in seconds): "))
GSjson = GSheet + '.json'
...
def gvHardStop():
    global HardStop
    HardStop = gsAccDsh.cell('F5').value
    HardStop = abs(float(HardStop.split('%')[0])/100)

def gvInitBalance():
    global InitBalance
    InitBalance = gsAccDsh.cell('F4').value
...
def gvVolatilities():
    ...
    ATR = pd.read_sql(query, dbConnection)
    ATR = ATR.set_index('Symbol')
    ATR = ATR.astype(float)
    ATR = ATR.T
    ATR = ATR.mean()
    query = "SELECT * FROM StDevReturns"
    StdDev = pd.read_sql(query, dbConnection)
    StdDev = StdDev.set_index('Symbol')
    StdDev = StdDev.astype(float)
    StdDev = StdDev.T
    StdDev = StdDev.mean()
    Volatilities = round((ATR+StdDev)/2, 5)
